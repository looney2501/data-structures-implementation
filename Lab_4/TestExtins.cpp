...
void test_functionalitate_extra()
{
	Multime m;
	assert(m.diferenta_max_min() == -1);
	m.adauga(1);
	assert(m.diferenta_max_min() == 0);
	for (int i = 1; i < 10; i++)
		m.adauga(1);
	assert(m.diferenta_max_min() == 0);
	for (int i = -100; i <= 100; i++)
		m.adauga(i);
	assert(m.diferenta_max_min() == 200);
	for (int i = 1; i <= 50; i++)
	{
		m.sterge(2 * i);
		m.sterge(-2 * i);
	}
	assert(m.diferenta_max_min() == 198);
	for (int i = -100; i <= 100; i++)
		m.sterge(i);
	assert(m.diferenta_max_min() == -1);
	for (int i = -100; i <= 100; i++)
